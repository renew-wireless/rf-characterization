...
class ofdmTxRx:
    ...
    def sfo_correction(self, rxSig_freq_eq, pilot_sc, pilots_matrix, n_ofdm_syms):
        ...
        pilot_shift_diff_mod = np.remainder(pilot_shift_diff, 64).reshape(len(pilot_shift_diff), 1, order="F")
        ...
        tmp = np.array(range(-32, 32)).reshape(len(range(-32, 32)), 1, order="F")
